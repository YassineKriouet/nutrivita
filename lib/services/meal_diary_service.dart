...
class MealDiaryService {
  ...
  /// Enhanced getUserMeals with recipe support and fiber data
  Future<List<Map<String, dynamic>>> getUserMeals({
    ...
    try {
      ...
      // Transform database meal data to UI-compatible format with recipe support and fiber data
      ...
      for (var entry in response) {
        ...
        for (var mealFood in mealFoods) {
          ...
          totalFiber += (mealFood['fiber_g'] as num? ?? 0)
              .toDouble(); // Add fiber calculation
        }
  ...
  bool _validateFoodData(Map<String, dynamic> food) {
    try {
      ...
      if (!hasFoodItemId && !hasRecipeId) {
        print('Food data missing both food_item_id and recipe_id');
        return false;
      }

      if (hasFoodItemId && hasRecipeId) {
        print(
          'Food data has both food_item_id and recipe_id - should only have one',
        );
        return false;
      }
  ...
  Future<void> updateMealFoodQuantity({
    ...
    try {
      ...
      if (mealFoodResponse['food_items'] != null) {
        ...
        // Calculate per 100g values for recipe including fiber
        final averageServingWeight = 250.0; // Assuming 250g per serving
