...
class QuestionnaireService {
  ...
  Future<String?> startAssessmentSession(String questionnaireType) async {
    try {
      ...
      // IMPROVED: Check for existing in_progress session first (for continuation)
      final existingActiveSession = await _supabase
          .from('assessment_sessions')
      ...
      if (existingActiveSession != null) {
        // Return the existing active session for continuation
        final sessionId = existingActiveSession['id'] as String;
        print('CONTINUATION: Found existing active session: $sessionId');

        // Trigger automatic calculations to ensure all calculated questions are up to date
        await autoCalculateResponses(sessionId);

        return sessionId;
      }

      // Check if user has already completed this questionnaire type
      final alreadyCompleted = await hasCompletedQuestionnaire(
        questionnaireType,
      );

      if (alreadyCompleted) {
        print(
          'User has already completed questionnaire type: $questionnaireType. Creating new session for potential retake.',
        );
      }

      // Create new session (for new questionnaires or retakes)
      print(
  ...
  // Enhanced Auto-calculate responses based on user data with MUST-specific logic
  Future<void> autoCalculateResponses(String sessionId) async {
    try {
      ...
      // Also handle client-side calculation for MUST questionnaire
      ...
      if (sessionInfo != null && sessionInfo['questionnaire_type'] == 'must') {
      ...
      try {
        ...
        if (sessionInfo != null &&
            sessionInfo['questionnaire_type'] == 'must') {
          await _handleMustCalculatedQuestion('must_bmi_calculated', sessionId);
          print('MUST SERVICE: Fallback calculation completed');
        }
      } catch (fallbackError) {
        print('MUST SERVICE: Fallback calculation also failed: $fallbackError');
      }
    }
  }
  ...
  // CRITICAL FIX: Enhanced completion logic for ESAS questionnaire (no scoring)
  Future<Map<String, dynamic>?> completeAssessment(String sessionId) async {
    try {
      ...
      // CRITICAL FIX: Handle ESAS questionnaire with special no-scoring logic
      int totalScore = 0;
      String riskLevel = 'completed';

      if (questionnaireType.toLowerCase() == 'esas') {
        // CRITICAL FIX: For ESAS questionnaire, no score calculation - consultation only
        print(
            'ESAS questionnaire completed - No scoring, consultation mode only');
        totalScore = 0; // No score for ESAS
        riskLevel = 'consultation_only'; // Special status for ESAS

        // Complete directly without BMI validation or scoring
        ...
        final updateResponse = await _supabase
            ...
            .update({
              'status': 'completed',
              'completed_at': completedAt,
              'total_score': 0, // Always 0 for ESAS
              'risk_level': 'consultation_only', // Special risk level for ESAS
              'recommendations':
                  'Risposte registrate per consultazione del paziente',
        ...
        print(
            'Successfully completed ESAS session in consultation mode: $sessionId');
        ...
        return {
          ...
          'completed_at': completedAt,
          'session_id': sessionId,
          'consultation_mode': true, // Flag for UI to handle differently
        };
      } else if (questionnaireType.toLowerCase() == 'must') {
        // CRITICAL FIX: For MUST questionnaire, calculate total score but don't show to user
        try {
          final scoreResponse = await _supabase.rpc(
            'calculate_questionnaire_score',
            params: {'session_uuid': sessionId},
          );

          totalScore = scoreResponse as int? ?? 0;

          // For MUST, store the score but mark as 'advisory' for UI logic
          riskLevel =
              'advisory'; // Special status indicating advisory message should be shown

          print(
            'MUST questionnaire completed - Score: $totalScore (stored but not shown to user)',
          );
        } catch (scoreError) {
          print('Error calculating MUST score: $scoreError');
          totalScore = 0;
          riskLevel = 'advisory';
        }
      } else if (questionnaireType.toLowerCase() == 'sarc_f') {
        // For SARC-F, just mark as completed with no scoring
        print('SARC-F questionnaire - completing without score calculation');
        totalScore = 0; // No score for SARC-F
        riskLevel = 'completed'; // Special status for SARC-F
      } else {
        // Calculate total score using database function for other questionnaires
        final scoreResponse = await _supabase.rpc(
          'calculate_questionnaire_score',
          params: {'session_uuid': sessionId},
        );
        ...
        // Determine risk level using database function
        try {
          final riskLevelResponse = await _supabase.rpc(
          ...
          riskLevel = riskLevelResponse as String? ?? 'low';
        } catch (riskError) {
          print('Error determining risk level: $riskError');
          riskLevel = 'low'; // Default fallback
        }
      }

      // CRITICAL FIX: Update session with completion data (for non-ESAS questionnaires)
      final completedAt = DateTime.now().toIso8601String();
      ...
      print('Successfully updated session to completed: $sessionId');
      ...
      return {
        ...
        'session_id': sessionId,
        'consultation_mode':
            questionnaireType.toLowerCase() == 'esas', // Special flag for ESAS
      };
    } catch (e) {
      print('Error completing assessment: $e');

      // Try to update status anyway if session exists
      try {
        await _supabase.from('assessment_sessions').update({
          'status': 'completed',
          'completed_at': DateTime.now().toIso8601String(),
          'total_score': 0,
          'risk_level': 'completed',
        ...
        print('Failed to update session status: $updateError');
        return null;
      }
    }
  }
