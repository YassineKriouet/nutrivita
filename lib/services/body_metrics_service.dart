...
class BodyMetricsService {
  ...
  Future<Map<String, dynamic>> getBodyMetricsSummary() async {
    try {
      ...
      // Get latest weight entry - USING PROPER SUPABASE METHODS
      ...
      double? bmi;
      final latestWeight = weightResponse?['weight_kg']?.toDouble();
      final height = profileResponse?['height_cm']?.toDouble();

      if (latestWeight != null && height != null && height > 0) {
        bmi = calculateBMI(weightKg: latestWeight, heightCm: height);
      }

      return {
        'latest_weight': weightResponse,
        'medical_profile': profileResponse,
        'weight': latestWeight ??
            profileResponse?['current_weight_kg']?.toDouble() ??
            0.0,
        'height': height ?? 0.0,
        'bmi': bmi,
        'body_fat_percentage':
            weightResponse?['body_fat_percentage']?.toDouble(),
        'muscle_mass_kg': weightResponse?['muscle_mass_kg']?.toDouble(),
        'waist_circumference_cm':
            weightResponse?['waist_circumference_cm']?.toDouble(),
        'hip_circumference_cm':
            weightResponse?['hip_circumference_cm']?.toDouble(),
        'lean_mass_kg': weightResponse?['lean_mass_kg']?.toDouble(),
        'fat_mass_kg': weightResponse?['fat_mass_kg']?.toDouble(),
        'cellular_mass_kg': weightResponse?['cellular_mass_kg']?.toDouble(),
        'phase_angle_degrees':
            weightResponse?['phase_angle_degrees']?.toDouble(),
        'hand_grip_strength_kg':
            weightResponse?['hand_grip_strength_kg']?.toDouble(),
        'last_updated':
            weightResponse?['recorded_at'] ?? weightResponse?['created_at'],
      };
    } catch (error) {
