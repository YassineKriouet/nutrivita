    ...
    try {
      ...
      return response.map((entry) {
        ...
        for (final mealFood in mealFoods) {
          final quantity = mealFood['quantity_grams']?.toDouble() ?? 0;
          final foodItem = mealFood['food_items'];

          if (foodItem != null) {
            foodNames.add(foodItem['name'] ?? 'Unknown Food');

            if (quantity > 0) {
              final caloriesPer100g =
                  foodItem['calories_per_100g']?.toDouble() ?? 0;
              final proteinPer100g =
                  foodItem['protein_per_100g']?.toDouble() ?? 0;
              final carbsPer100g = foodItem['carbs_per_100g']?.toDouble() ?? 0;
              final fatPer100g = foodItem['fat_per_100g']?.toDouble() ?? 0;
    ...
    for (final entry in mealEntries) {
      ...
      for (final mealFood in mealFoods) {
        ...
        if (foodItem != null && quantity > 0) {
          final caloriesPer100g =
              foodItem['calories_per_100g']?.toDouble() ?? 0;
          final proteinPer100g = foodItem['protein_per_100g']?.toDouble() ?? 0;
          final carbsPer100g = foodItem['carbs_per_100g']?.toDouble() ?? 0;
          final fatPer100g = foodItem['fat_per_100g']?.toDouble() ?? 0;
